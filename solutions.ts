// Problem 1:Create a TypeScript function filterEvenNumbers that accepts an array of numbers and returns a new array containing only the even number
function filterEvenNumbers (arr:number[]):number[]{
 return arr.filter((elem: number): boolean => elem % 2 === 0)
}
...
interface Book {
    ...
    const toggleReadStatus= <T extends Book> (bookInfo :T) =>{
        return{
            ...bookInfo,
            isRead : true,
        }
